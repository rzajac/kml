...
func Test_Parse(t *testing.T) {
	...
	assert.Exactly(t, "document name", name.ContentString())
	...
	assert.Exactly(t, "01020304", color.ContentString())
	...
	assert.Exactly(t, "1", scale.ContentString())
	...
	assert.Exactly(t, 0, color.AttributeCnt())
	assert.Exactly(t, 0, color.ChildCnt())
	assert.Exactly(t, "05060708", color.ContentString())
	...
	assert.Exactly(t, "3.5", width.ContentString())
	...
	assert.Exactly(t, 0, color.AttributeCnt())
	assert.Exactly(t, 0, color.ChildCnt())
	assert.Exactly(t, "090a0b0c", color.ContentString())
	...
	assert.Exactly(t, "1", outline.ContentString())
	...
	assert.Exactly(t, 0, name.AttributeCnt())
	assert.Exactly(t, 0, name.ChildCnt())
	assert.Exactly(t, "folder name", name.ContentString())
	...
	assert.Exactly(t, "snip", snippet.ContentString())
	...
	assert.Exactly(t, 0, name.AttributeCnt())
	assert.Exactly(t, 0, name.ChildCnt())
	assert.Exactly(t, "placemark name", name.ContentString())
	...
	assert.Exactly(t, "<b>placemark description</b>", desc.ContentString())
	...
	assert.Exactly(t, "#sty_0", styleUrl.ContentString())
	...
	assert.Exactly(t, "1", ts.ContentString())
	...
	assert.Exactly(t, "0.1,0.2,0.3 1.1,1.2,1.3", cor.ContentString())
}
